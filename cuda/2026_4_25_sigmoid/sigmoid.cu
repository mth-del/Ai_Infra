...
__global__ void sigmoid_f32x4_kernel(float *x, float *y, int N){
    // every thread deal with 4 factor
...
__global__ void sigmoid_f16x8_kernel(half *x, half *y, int N){
    ...
    reg_x_3.x = __hmin(__hmax(reg_x_2.x, MIN_EXP_F16), MAX_EXP_F16);
    reg_x_3.y = __hmin(__hmax(reg_x_2.y, MIN_EXP_F16), MAX_EXP_F16);
...
__global__ void sigmoid_f16x8_pack_kernel(half *x, half *y, int N){
    ...
    // 一次性把 128 bit（16 字节）结果写回到 y[idx] 开始的位置。

...
//  在 GPU 上调用，并建议编译器 内联，减少函数调用开销。
__device__ __forceinline__ float warp_reduce_sum_f32(float val){
#pragma unroll
    for (int mask = kWarpSize >> 1; mask >= 1; mask >>= 1) {
        // 32 个 lane 都参与 shuffle（全 warp 活跃掩码）。
...
// RMS Norm x:NxK(k=256<1024), y':NxK, y'= x/rms(x) echo row
// 1/rms(x) = rsqrtf(sum^2/K) each row
...
__global__ void rms_norm_f32_kernel(float *x, float *y , float g, int N, int K){
    ...
    if(tid == 0){
        s_variance = rsqrt(variance / (float) K + eplison);
    }

    __syncthreads();
    if(idx < N*K)
        // value * s_variance：把输入按该行的 RMS 做归一化。
        // 再乘 g：RMSNorm 的缩放参数（gamma）。
...
// #pragma unroll 是给编译器的提示：把紧跟着的循环展开（loop unrolling），尽量减少循环控制开销并提升指令级并行。
#pragma unroll
    for(int mask = kWarpSize >> 1; mask >= 1; mask >>=1){
...
__device__ float block_reduce_sum_f16_f32(half val){
    ...
    val_f32 = warp_reduce_sum_f16_f32<WARP_SIZE>(val);
    return val_f32;
}
...
__global__ void rms_norm_f16_f32_kernel(half *x, half *y, float g, int N, int K){
    ...
    if (tid == 0)
        s_variance = rsqrtf(variance / (float)K + epsilon);
    // wait for s_variance in shared memory in shared memory to be ready for all threads
    __syncthreads();

...
# 第一次跑的时候会去找对应的环境然后进行编译
# 编译之后利用python将.so加载到对应的模块
lib = load(
...
def run_benchmark(
    perf_func: callable,  # 测性能的函数
    a: torch.Tensor,      # 参与运算的两个张量
    b: torch.Tensor,
    tag: str,             # 字符串标签
    out: Optional[torch.Tensor] = None, # 可选输出张量
    warmup: int = 10,     # 预热
    iters: int = 1000,    # 正式计数器
    show_all: bool = False,
):
...
for S, K in SKs:
    ...
    c = torch.zeros_like(a).cuda().float().contiguous()  # 输出缓冲
